...
export class RecordClient {
  ...
  public addRecord(params: { app: AppID; record?: RecordForParameter }): Promise<{ id: string; revision: string }> {
    const path = this.buildPathWithGuestSpaceId({
      endpointName: 'record',
    })
  ...
  public async addRecords(params: { app: AppID; records: RecordForParameter[] }): Promise<{
    ids: string[]
    revisions: string[]
  ...
  public deleteRecords(params: { app: AppID; ids: RecordID[]; revisions?: Revision[] }): Promise<{}> {
    const path = this.buildPathWithGuestSpaceId({
      endpointName: 'records',
    })
  ...
  private async getAllRecordsRecursiveWithOffset<T extends Record>(
    ...
    const conditionQuery = condition ? `${condition} ` : ''
    const query = `${conditionQuery}${orderBy ? `order by ${orderBy} ` : ''}limit ${GET_RECORDS_LIMIT} offset ${offset}`
    const result = await this.getRecords<T>({ ...rest, query })
    const allRecords = records.concat(result.records)
    if (result.records.length < GET_RECORDS_LIMIT) {
      return allRecords
    ...
    return this.getAllRecordsRecursiveWithOffset(params, offset + GET_RECORDS_LIMIT, allRecords)
  }
  ...
  private async getAllRecordsRecursiveByCursor<T extends Record>(id: string, records: T[]): Promise<T[]> {
  ...
  public async addAllRecords(params: {
    ...
    if (!params.records.every((record) => !Array.isArray(record) && record instanceof Object)) {
  ...
  private async addAllRecordsRecursive(
    ...
    const CHUNK_LENGTH = this.bulkRequestClient.REQUESTS_LENGTH_LIMIT * ADD_RECORDS_LIMIT
    ...
    try {
      ...
      throw new KintoneAllRecordsError({ records: results }, records, numOfAllRecords, e, ADD_RECORDS_LIMIT)
    }
  ...
  private async addAllRecordsWithBulkRequest(params: { app: AppID; records: RecordForParameter[] }): Promise<
    Array<{
      id: string
      revision: string
    }>
  > {
    const separatedRecords = this.separateArrayRecursive(ADD_RECORDS_LIMIT, [], params.records)
    ...
    const results = (await this.bulkRequestClient.send({ requests })).results as Array<{
      ids: string[]
      revisions: string[]
    }>
    return results
  ...
  private async updateAllRecordsRecursive(
    ...
    try {
      ...
      throw new KintoneAllRecordsError({ records: results }, records, numOfAllRecords, e, UPDATE_RECORDS_LIMIT)
    }
  ...
  private async updateAllRecordsWithBulkRequest(params: {
    ...
    const separatedRecords = this.separateArrayRecursive(UPDATE_RECORDS_LIMIT, [], params.records)
    const requests = separatedRecords.map((records) => ({
      method: 'PUT',
      endpointName: 'records' as const,
      payload: {
  ...
  private async deleteAllRecordsRecursive(
    ...
    const CHUNK_LENGTH = this.bulkRequestClient.REQUESTS_LENGTH_LIMIT * DELETE_RECORDS_LIMIT
    const { app, records } = params
    const recordsChunk = records.slice(0, CHUNK_LENGTH)
    if (recordsChunk.length === 0) {
      return {}
    ...
    try {
      ...
      throw new KintoneAllRecordsError({}, records, numOfAllRecords, e, DELETE_RECORDS_LIMIT)
    }
  ...
  private async deleteAllRecordsWithBulkRequest(params: {
    ...
    const separatedRecords = this.separateArrayRecursive(DELETE_RECORDS_LIMIT, [], params.records)
    const requests = separatedRecords.map((records) => ({
      method: 'DELETE',
      endpointName: 'records' as const,
      payload: {
  ...
  private separateArrayRecursive<T>(size: number, separated: T[][], array: T[]): T[][] {
    const chunk = array.slice(0, size)
    if (chunk.length === 0) {
      return separated
    }
    return this.separateArrayRecursive(size, [...separated, chunk], array.slice(size))
  }
  ...
  public deleteRecordComment(params: { app: AppID; record: RecordID; comment: CommentID }): Promise<{}> {
    const path = this.buildPathWithGuestSpaceId({
      endpointName: 'record/comment',
    })
    return this.client.delete(path, params)
